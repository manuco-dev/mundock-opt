...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadPromises = files.map(async (file: any) => {
      ...
      const isVideo = allowedVideoTypes.includes(fileExt.toLowerCase());
      
      if (!isImage && !isVideo) {
        throw new Error(`Tipo de archivo no permitido: ${fileExt}`);
      }
      
      // Validar tamaño del archivo (máximo 20MB para imágenes, 100MB para videos)
      ...
      const fileUrl = `/api/uploads/hero-images/${fileName}`;
    ...
    console.error('Error processing hero-images upload:', error);
    return NextResponse.json(
...
export const config = {
  api: {
    bodyParser: false,
    responseLimit: '100mb', // Aumentado para permitir videos
  },
};