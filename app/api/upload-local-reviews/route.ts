...
// Directorio base para almacenar archivos de reseñas
const UPLOAD_DIR = path.join('/data', 'files', 'uploads', 'reviews');
...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadPromises = files.map(async (file: any) => {
      ...
      const fileUrl = `/api/uploads/reviews/${fileName}`;
...
export const config = {
  api: {
    bodyParser: false,
    responseLimit: '50mb', // Aumentado para permitir videos
  },
};