...
interface AlternativeImageUploadProps {
  value: string[];
...
export default function AlternativeImageUpload({
  value = [],
  onChange,
  onRemove,
  disabled = false,
  ...
  const uploadToLocalStorage = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('/api/upload-local-properties', {
        method: 'POST',
        body: formData
      });
  ...
  }, [disabled, onChange, value, folder, onUploadComplete, onUploadError, onUploadStart]);
